...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{img/dex_format.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/build_flow.png}
...
%
\item \verb|apkbuilder| - pakuje nieskompilowane zasoby (obrazki), skompilowane zasoby i pliki \verb|.dex| do pliku \verb|.apk|.
%
...
%
Nasz kod w formie skompilowanej jest zapisany w bajtkodzie.
Dzięki bajtkodowi i maszynie wirtualnej, która go przetwarza, zbudowana tak aplikacja jest w stanie uruchomić się na dowolnym środowisku pod warunkiem, że posiadamy maszynę wirtualną działającą pod ów środowiskiem.
Całą ideą języka Java jest \say{Zbuduj raz, działaj wszędzie} (ang. \say{"Write once, run anywhere"})(WORA)\cite{WORA}. W przypadku Androida  mamy jednak do czynienia ze specjalnym bajtkodem. Wynika to z pewnej specyfiki urządzeń mobilnych i ograniczeń praw patentowych. W rozdziale \ref{apk_dex_how_to} został opisany proces kompilacji aplikacji.
%
\section{SMALI}
\label{smali}
%
Smali w tłumaczeniu islandzkim to właśnie asembler\cite{SmaliWiki}. Smali nie jest skomplikowanym językiem składniowo. Konstrukcje są prymitywne i bardzo czytelne. Częściowo słowa kluczowe pokrywają się z tymi, które znajdziemy w op-kodach maszyny Dalivik, jak i bezpośrednio w słowach kluczowych języka Java.  Poniżej znajduje się kod smali, który wyświetli na ekranie \say{Hello World!}
...
Dalvik i ART to dwie maszyny wirtualne działające na systemie Android. Dalvik był domyślną maszyną dla Androida do wersji 4.4. Od wersji 5.0 nową domyślną maszyną jest ART. Nowa maszyna jest wstecznie kompatybilna i działa ze starszymi wersjami aplikacji, które zostały zbudowane zanim był dostępny ART. Istotną różnicą między typową maszyna wirtualną Javy a Dalvikiem jest to, że Dalvik posiada architekturę nastawioną na rejestry, a nie stos. Podczas procesu budowania aplikacji na system Android kompilujemy kod za pomocą kompilatora Javy do plików \verb|.class|, a następnie za pomocą narzędzia \verb|dx| następuje konwersja plików \verb|.class| do jednego pliku \verb|.dex|. Podczas tej konwersji odpowiednie instrukcje bajtkodu Javy są zamieniane na odpowiadające im instrukcje w bajtkodzie Dalvika.
...
\end{figure}
%
Jedną z istotnych zmian jakie wprowadził ART to \verb|AOT|, czyli \say{Ahead-of-time compilation}. Dzięki temu zabiegowi aplikacja dodatkowo jest optymalizowana w trakcie instalacji na urządzeniu użytkownika. Kosztem jest dłuższy czas instalacji, jednak warto poświęcić na to jednorazowo czas i zyskać wydajniejszą aplikację.
Tak zoptymalizowana aplikacja jest zapisywana w formacie \verb|.odex|. Format ten nie jest przenośny między wirtualnymi maszynami. Z założenia zoptymalizowany plik jest tworzony na tej wirtualnej maszynie, na której będzie wykonywany.\\
%
...
%
Celem projektu jest zaprezentowanie jednej z technik, jaką możemy się
posłużyć, by zabezpieczyć nasz kod źródłowy. W projekcie został wykorzystany sposób przez zaszyfrowanie skompilowanego kodu i w trakcie działania odszyfrowanie go. Dzięki takiemu rozwiązaniu w naszym 
\verb|.apk| nie znajdują się bezpośrednio wrażliwe dane, tylko zostają 
bardziej ukryte. Oczywiscie ta metoda nie daje 100% gwarancji bezpieczeństwa a jedynie znacząco utrudnia napastnikowi wydobycie naszych
cennych danych.\\
Projekt został zrealizowany w języku Java z wykorzystaniem Android SDK\cite{AndroidSDK}
%
%
%%%%%%%%%%%
...
Aby urochomić projekt należy na swoim systemie zainstalować Android Studio\cite{AndroidStudio} wraz z Android API 26. Następnie wystarczy z poziomu Android Studio uruchomić projekt. Projekt został przygotowany w taki sposób, aby nie trzeba było wykonać skomplikowanej inicjalizacji projektu.
%
%
%
%
...
%
W źródłach aplikacji mamy plik \verb|Encrypter.java|, który jest odpowiedzialny za szyfrowanie plików wejściowych. Wykorzystane jest tam proste szyfrowanie z wykorzystaniem operacji \verb|XOR|.
W trakcie budowania aplikacji w pliku \verb|build.gradle| zaraz po skompilowaniu plików źródłowych naszej aplikacji do postaci \verb|.class|, wykonujemy konwersję z \verb|.class| do \verb|.dex| tych plików których będziemy chcieli zabezpieczyć. W tym przypadku jest to plik: \verb|SecuredData.java|. Następnie przekonwertowany tak plik szyfrujemy i przenosimy nasz plik do zasobów \verb|assets|. Na koniec zaszyfrowany plik \verb|.class| usuwamy, aby nie trafił do ostatecznej aplikacji.\\
Proces, który przeprowadziliśmy na pliku wynika bezpośrednio ze sposobu w jaki Android SDK przygotowuje aplikację, który był poruszony w: \ref{prepare_process}.\\
%
%
%
\newpage
